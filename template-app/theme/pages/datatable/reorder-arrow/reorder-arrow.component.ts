...
@Component({
    selector: ".m-wrapper",
    templateUrl: "./reorder-arrow.component.html",
    styleUrls: ['./reorder-arrow.component.scss'],
    encapsulation: ViewEncapsulation.None
})

export class ReorderArrowComponent implements OnInit {
    ...
    initTable() {
        ...
        this.customConfig = {
            ...
            columns: [
                ...
                {
                    title: this._services.translate.instant('Rearrange'),
                    orderable: !1,
                    data: 'sortBy',
					width: 100,
					render: function(data, type, row) {
						const up = `<i class="fa fa-arrow-up btn-up cursor" data-name="${row.name}"></i>`;
						const down = `<i class="fa fa-arrow-down btn-down cursor m--margin-left-20" data-name="${row.name}"></i>`
						return (data !== 1 ? up : '') + (data !== that.data.length ? down : '');
					}
				},
                {
    ...
    initTableEvents() {
        ...
        this.dataTable.on("click", ".btn-up", function() {
            let name = $(this).data('name').toString();
            let data = that.data.find(d => d.name === name);
            that.reorder(data, 'UP');
        })
        this.dataTable.on("click", ".btn-down", function() {
            let name = $(this).data('name').toString();
            let data = that.data.find(d => d.name === name);
            that.reorder(data, 'DOWN');
        })
    }
    ...
    reorder(data, type) {
        ...
        for(let x = 0; x < this.data.length; x++) {
            if(this.data[x].name === data.name) {
                newArray.push(this.data[x]);
                if(type === 'UP') {
                    newArray.push(this.data[x-1]);
                } else if (type === 'DOWN') {
                    newArray.push(this.data[x+1]);
                }
                break;
        ...
        this._services.http(`put`, 'api', body, true, true, false, false).subscribe((response: any) => {
